    ...
    facilities=(
        SiteFacility(
            ...
            cooling_model_citation=(
                ...
                "blowdown (A2: OHD000001 was WITHDRAWN 2026-07-21, Ohio EPA declined to finalize it, so no "
                "general-permit coverage will ever exist), the outcome is a `gap` that KEEPS this "
                "[reference] pin — "
    ...
    facilities=(
        SiteFacility(
            ...
            cooling_model_citation=(
                ...
                "file (OHD000001 was WITHDRAWN 2026-07-21 — Ohio EPA declined to finalize it, so it never "
                "covered this or any facility; no facility-specific "
    ...
    facilities=(
        SiteFacility(
            ...
            disclosure_citation=(
                ...
                "Roads), Sidney — the address AS DISCLOSED; that situs was retired by the "
                "Consolidation Plat (V37 P50), whose LOT NUMBER does not reconcile across the "
                "record — this line's 7658 against the 7647 the City's own Res. 14-26 states in "
                "accepting the plat, against the 7646/7647/7648 the Development Agreement's "
                "Exhibit A recites for the three deeds (#1998, [open]). The parcel of record is "
                "now 26-03-201-002, situs "
                ...
                "advocacy site, and all three were wrong): the CRA was ESTABLISHED city-wide by "
                "Resolution 84-22, adopted 2022-10-10 (ODOD #149-72424-06), and EXPANDED by "
                "Resolution 69-25, adopted 2025-09-08, which also added the 30-year megaproject "
                "tier its §4 had never carried — there is no 'Res. 18-25 of October 2025', which is "
                "why it was never findable, and 69-25 is not itself the designating act (#1998; "
                "this line said it was until both instruments were read). The abatement agreement is "
...
_WILMINGTON = SiteProfile(
    ...
    facilities=(
        SiteFacility(
            ...
            it_load_source=ItLoadGrounding.SCREENING,
            it_load_citation=(
                "[inference] SCREENING bracket — NOT a disclosure. A DISCLOSED LOAD FIGURE NOW "
                "EXISTS AND IS DELIBERATELY NOT ADOPTED HERE (#1469): PJM's TEAC Dayton "
                "Supplemental filing of 2026-07-07 carries M-3 Need Dayton-2025-002, 'AES Ohio "
                "has a customer request for service in the vicinity of its Clinton Substation "
                "near Wilmington, OH', ramping 35 MW (1/2028) -> 100 MW (4/2028) -> 300 MW "
                "(1/2029) -> 500 MW (1/2030). It is not adopted for three reasons, each "
                "sufficient: (a) it is TOTAL REQUESTED NEW LOAD at a delivery point — facility "
                "demand, not IT load (read back through this repo's own PUE band 1.13-1.45 it "
                "implies ~345-442 MW of IT, which lands inside the bracket below and under its "
                "480.1 high, so the two methods AGREE rather than conflict); (b) it is a request "
                "with a 2030 terminal year, not installed capacity; and (c) decisively, THE "
                "FILING NAMES A SUBSTATION AND A CITY, NEVER A CUSTOMER — the Clinton Substation "
                "parcel is 53 m from the nearest Ardent/TAC petitioned-rezoning tract and 1.03 mi "
                "from the nearest AWS-owned Cosler Farm holding, so the 500 MW cannot be assigned "
                "to this campus on the present record (the Van Wert #1408 shape: an instrument "
                "that documents the load but not the customer does not lift the facility). "
                "Records: data/extracted/grid/wilmington/clinton-delivery-point.interconnection.yaml, "
                "data/extracted/wilmington/power-watch.yaml. Primary basis = the network "
                ...
                "imply a larger N+1 backup envelope. RETIRED signal: the widely repeated '1.5 GW' is NOT "
                "this campus's figure at all and must not be carried as one — #1469 traced it to the SAME "
                "PJM deck's PREVIOUS slide, Need Dayton-2025-001, 'in the vicinity of its Fayette "
                "Substation in Jeffersonville, OH' (Fayette County, ~27 mi away), 35 MW (9/2026) -> 480 MW "
                "(9/2028) -> 1.5 GW (1/2031); AES Ohio's own project pages for the two Fayette customer "
                "feeds (OPSB 25-0742/25-0743-EL-BLN) attach it to 'the Jeffersonville, Ohio area' in its "
                "own words. A Dayton-market press ranking mislabelled it 'Wilmington' and this repo "
                "repeated the mislabel. Replace this bracket with the disclosed load the moment an "
                "instrument names THE CAMPUS's MW — the highest-value candidate is the OPSB filing for "
                "PJM's 'Customer Service Lines from Clinton 1 & 2' ($20M, two ~2-mile 345 kV feeds to an "
                "as-yet-unnamed customer substation, ISD 2028-12-31), whose affected-property table would "
                "name the served parcels. "
...
_BOWLING_GREEN = SiteProfile(
    ...
    facilities=(
        SiteFacility(
            ...
            cooling_model_citation=(
                ...
                "B5 (#1685) RAN that tension through the A3 cooling harness and the pin SURVIVES, but "
                "as a reservation_conflict rather than an untested claim: the district-linked ~600,000 "
                "gpd design commitment is a demand signal independent of Meta's own account of its "
                "cooling, and is disproportionate to 'no operational water'. It cannot re-archetype the "
                "pin — a negotiated ceiling is not a withdrawal or discharge instrument — and the "
                "harness confirmed there is no instrument to be had: the campus buys finished water "
                "(so the Ohio DNR withdrawal registry, which records withdrawals from waters of the "
                "state, carries no Meta/Liames/Accordion/NWWSD registration in Wood County at all), and "
                "a full ECHO sweep of the county finds every campus-linked record under the "
                "CONSTRUCTION stormwater general permit with no process outfall and no DMR. With "
                "OHD000001 withdrawn 2026-07-21, an INDIVIDUAL NPDES permit is the only remaining "
                "instrument on the DIRECT-DISCHARGE path — but that is not the only path, and here "
                "it may well be the wrong one: this campus's discharge route is not established, and "
                "a facility blowing down to a sanitary sewer files no DMR at all and is disclosed "
                "instead by the City/district industrial-pretreatment (IU) permit and sewer-use "
                "agreement, which is what the records ask actually seeks. See "
                "data/reference/oepa/cooling-reconciliation.yaml. "
    ...
    serving_utility_citation=(  # [reference] not corpus
        "BOWLING GREEN IS A TWO-GRID SITE AND THIS KNOB CARRIES ONLY ONE OF THEM — the SITE's "
        "utility, which is the City of Bowling Green's municipal electric system (EIA-861 2024 "
        "Service_Territory, utility #2054, Municipal, BA=PJM, an American Municipal Power member; "
        "resolved #1434). It is deliberately NOT re-pointed at the campus's utility: the load "
        "shares, energy-burden and consumer-energy feeds all key on this number for the city's own "
        "14,850 retail customers, and moving it to flatter one facility's denominator would "
        "falsify the backdrop for all of them (#1440). "
        "THE META CAMPUS IS ON A DIFFERENT GRID. Middleton Twp sits in The Toledo Edison Company's "
        "territory (FirstEnergy / PJM ATSI) — and that is now tariff-grounded rather than "
        "press-sourced: Toledo Edison's own filed P.U.C.O. No. 8, Original Sheet 3, DEFINITION OF "
        "TERRITORY, effective 2026-03-01, names Wood County among the ten counties of 'Company's "
        "Territory' (data/documents/grid/bowling-green/TE-2026-Electric-Service.pdf). A county is "
        "not a parcel, so the parcel-level certified-territory read stays [open]. The campus is "
        "designed self-powered BEHIND THE METER by the Apollo plant (OPSB 25-0973-EL-BLN), whose "
        "Condition 15 BARS any interconnection with the PJM Transmission System — so its modelled "
        "grid draw is ~0 and what Toledo Edison actually sells it is [open]. "
        "READ THE PER-UTILITY LOAD SHARE BELOW AS A MAGNITUDE COMPARISON ONLY (the campus dwarfs "
        "the muni's retail at ~3.6x on energy — the point being that the muni could never serve "
        "it), NOT as the campus's serving-utility ratio; a per-facility serving-utility "
        "denominator is a grid-model follow-up. The city's OTHER data centre — the Oppidan "
        "colocation building in the Woodbridge Business Park, ~6 MW average / ~8 MW peak — IS on "
        "the muni ([inference], high confidence: five city-owned overhead electric features cross "
        "the parcel in the City's own GIS, the parcel is city-zoned IE under annexation ordinances "
        "8597/8824, and the BG utilities director called it 'a steady electric customer for the "
        "city'; the BPU minutes remain the instrument). Full per-load determination, the in-force "
        "Toledo Edison sheets (Rate GT, ESR §II.G) and the pending FirstEnergy Schedule DCT "
        "(PUCO 26-0697-EL-ATA) are at data/extracted/grid/bowling-green/ (#1440)."
    ),
