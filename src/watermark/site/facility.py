...
def build_facility_feed(settings: Settings | None = None) -> list[FacilityItem] | None:
    ...
    (modeled) campus; each row resolves its geometry link (facility-level, else the site default).
    """
    ...
    for i, fac in enumerate(profile.facilities):
        ...
        items.append(
            FacilityItem(
                ...
                # Exactly one of these grounds a disclosed load (SiteFacility-validated); [open]
                # loads carry neither, so this is None then.
                it_load_citation=fac.air_permit_citation or fac.it_load_citation,
