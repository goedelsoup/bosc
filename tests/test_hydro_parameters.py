...
wins over the table, and lock the right-sized (2-sig-fig) reported peak.
"""
...
def test_accessors_return_the_cited_reference_values() -> None:
    s = Settings()
    assert params.peak_factor(settings=s) == pytest.approx(484.0)
    assert params.initial_abstraction_ratio(settings=s) == pytest.approx(0.2)
    assert params.default_manning_n(settings=s) == pytest.approx(0.04)
    assert params.dilution_bands(settings=s) == (pytest.approx(1.0), pytest.approx(10.0))


def test_committed_yaml_is_coupled_to_the_in_code_defaults() -> None:
...
def test_ia_ratio_override_beats_the_table() -> None:
    ...
    default = float(excess_rainfall(cumulative, 80.0)[-1])
    low_ia = float(excess_rainfall(cumulative, 80.0, ia_ratio=0.05)[-1])
    assert low_ia > default > 0.0


def test_peak_factor_override_beats_the_table() -> None:
...
def test_manning_n_override_beats_the_table() -> None:
    ...
    tabled = routing.route(inflow, length_ft=8000.0, slope=0.001, dt_hr=0.1)
    rough = routing.route(inflow, length_ft=8000.0, slope=0.001, dt_hr=0.1, manning_n=0.1)
    assert not np.allclose(tabled, rough)


def test_reported_peak_is_stored_to_two_significant_figures() -> None:
    ...
    h = simulate_runoff(area_acres=340.0, curve_number=86.0, tc_hr=0.6, storm_depth_in=4.5)
    assert h.peak_cfs == params.round_sig(h.peak_cfs)  # already 2 sig figs
    assert h.peak_cfs == pytest.approx(max(h.flows_cfs), rel=0.01)  # ~= the true series max
