...
def test_contract_version_bumped() -> None:
    """This feed landed at 1.22.0 (additive/MINOR); the contract has since advanced to 1.32.1
    ...
    defense-contractors federal-award join 1.32.0 #1662, then the hydrology-scenarios
    seasonal-floor + campus-discharge fields 1.32.1 #1633)."""
    assert CONTRACT_VERSION == "1.32.1"
...
def test_reference_export_emits_the_seasonal_field(lima_bundle: Path) -> None:
    ...
    assert manifest["contract_version"] == "1.32.1"
