...
// engine (Postgres) production uses, not a SQLite stand-in. The schema is the committed migration,
// so a schema/store drift fails a test. `fakePg()` is async because pglite initializes the WASM DB.

// Apply every committed migration in filename order, so a schema/store drift (or a missing later
// migration) fails a test — the same way the Lakebase migration runner applies them in sequence.
...
/** A `PgLike` whose `raw` pglite handle is exposed for direct seeding/inspection in a test. */
export interface FakePg extends PgLike {
  raw: PGlite;
}

function adapt(exec: Pick<PGlite, "query">): PgLike {
  return {
    query: async <T = unknown>(text: string, params: unknown[] = []) =>
      (await exec.query<T>(text, params)).rows,
    // pglite's `transaction(fn)` gives a tx handle with the same `query`; a throw rolls it back.
    begin: <T>(fn: (tx: PgLike) => Promise<T>) =>
      (exec as PGlite).transaction(async (tx) => fn(adapt(tx))) as Promise<T>,
  };
}

// Booting a fresh pglite WASM instance per test is slow enough to trip the default per-test timeout
// under parallel workers, so we reuse one instance per Vitest worker and reset the schema between
// calls. Tests within a file run sequentially, and each worker has its own module instance, so the
// singleton never leaks state across concurrently-running tests.
let sharedDb: PGlite | undefined;

/** Build an in-memory Postgres with the Stories schema applied (a pristine schema per call). */
export async function fakePg(): Promise<FakePg> {
  if (!sharedDb) sharedDb = await PGlite.create();
  const db = sharedDb;
  await db.exec("DROP SCHEMA public CASCADE; CREATE SCHEMA public;"); // pristine, drops all prior tables
  await db.exec(migrationSql());
  return { raw: db, ...adapt(db) };
}
