...
describe("validateStoryDocument", () => {
  it("accepts a doc of only the closed vocabulary, including nested blocks", () => {
    const value = doc([
      ...
      { type: "list", ordered: false, items: [[{ type: "atom", handle: "record:lima:x.deed.yaml" }]] },
  ...
  it("rejects a malformed shape (missing version, bad heading level, non-inline child)", () => {
    expect(validateStoryDocument({ blocks: [] })).toBeNull();
    expect(
      validateStoryDocument(doc([{ type: "heading", level: 1, children: [] } as unknown as SdmBlock])),
    ).toBeNull();
    expect(
      validateStoryDocument(
        doc([{ type: "paragraph", children: [{ type: "text" }] } as unknown as SdmBlock]),
      ),
    ).toBeNull();
  });
});
...
// --- the resolver seam ----------------------------------------------------------------------
describe("sdmHandles", () => {
  it("collects every atom handle depth-first, including nested in lists and blockquotes", () => {
    const value = doc([
      { type: "atom", handle: "record:lima:a" },
      { type: "blockquote", children: [{ type: "atom", handle: "entity:lima:b" }] },
      {
        type: "list",
        ordered: true,
        items: [
          [{ type: "atom", handle: "record:lima:c" }],
          [{ type: "paragraph", children: [{ type: "text", value: "x" }] }],
        ],
      },
    ]);
    expect(sdmHandles(value)).toEqual(["record:lima:a", "entity:lima:b", "record:lima:c"]);
  });
});
...
describe("resolveSdmAtoms / sdmIsResolvable", () => {
  ...
  it("resolves a live handle to its atom and dedupes repeats", () => {
    const value = doc([
      { type: "atom", handle: "record:lima:x.deed.yaml" },
      { type: "atom", handle: "record:lima:x.deed.yaml" },
    ]);
  ...
  it("surfaces the failure reason for a dangling handle and fails the write-time gate", () => {
    const value = doc([{ type: "atom", handle: "record:lima:retired.deed.yaml" }]);
