 ...
 *
 * An explicit override is **exclusive** (#2002): when it is set, (3)/(4) are not consulted at
 * all. Setting it means "read this bundle root", and a slug it does not carry must resolve to
 * nothing rather than to whatever a previous `watermark export` left on this machine.
 *
...
function candidateDirs(slug: string): string[] {
  ...
  if (override) {
    ...
    // EXCLUSIVE, and returning early is the whole point (#2002). This used to be merely the FIRST
    // candidate, so a slug the override root did not carry fell through to the git-ignored
    // `data/site/bundles/<slug>` that any local `watermark export` populates. A test that pinned a
    // temp fixture then silently read the developer's stale export instead — green in CI, where
