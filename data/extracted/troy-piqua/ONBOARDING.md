...
- [x] **Economics** — Miami County baseline + consumer-energy ran (high-confidence: BLS QCEW 2023 / Census ACS5). The signature is **mid-size manufacturing** (the Hobart / auto-parts base), the upstream-mainstem complement to Butler's heavy steel. RSEI toxics ran (Miami Co, 40 facilities / 33 scored; top by modeled Score: Hobart Brothers filler-metals — the manufacturing base named above). grid-profile ran on the now-pinned serving utility **Dayton Power & Light** (AES Ohio, EIA-861 #4922; PJM DAY / PUCO) — confirmed for #830 as both the county-dominant IOU (Troy) AND the Klondike campus's disclosed intended long-term utility (a **proposed** 40-yr AES Ohio franchise ordinance, first reading 3-1 on 2026-07-07, **pending adoption**). The City of Piqua municipal (#15095) serves the City + campus construction power; it is a **full-form** EIA-861 filer (checked: absent from Short_Form_2024), so the "861S short-form" watch resolves negative. `load_share` now populated. **Evidence:** [`grid-profile.yaml`](../../reference/eia/troy-piqua/grid-profile.yaml) + [`data-centers.md`](data-centers.md); primary sources: [EIA-861 2024 bulk file](https://www.eia.gov/electricity/data/eia861/), [City of Piqua Power System (piquaoh.gov/228)](https://www.piquaoh.gov/228/Power-System), and Dayton Daily News, "Piqua takes step toward powering planned data center" (2026-07-09). [verified]
...
- [x] **Per-jurisdiction GIS** — **Miami County parcels WIRED (#1483):** `MIAMI_PARCEL_SCHEMA` over the county AGOL `parcel_joined` FeatureServer (org `wCWf4EGMg4PzHwzA`), `parcels_url` + `gis_parcel` set; same-name trap cleared (situs Piqua OH 45356, FIPS 39109, WKID 3735). Flood = national NFHL (wired). **City of Piqua zoning WIRED (#830):** `PIQUA_ZONING_SCHEMA` over the City AGOL `Zoning_Districts_public_view/17` (org `kZPPWTIJ6kOFJTWc`), `zoning_url` + `gis_zoning` set; situs verified from a live sample — campus parcel `N44-101770` → `IH` (Industrial Heavy). District catalog + provenance committed under [`data/reference/troy-piqua-gis/`](../../reference/troy-piqua-gis/README.md); live endpoint [`Zoning_Districts_public_view/FeatureServer/17`](https://services8.arcgis.com/kZPPWTIJ6kOFJTWc/arcgis/rest/services/Zoning_Districts_public_view/FeatureServer/17).
...
`toxic_corridor_bbox` open; the Piqua-muni vs AES Ohio split is **DONE + confirmed (#830)** — utility pinned to
**DP&L #4922** (PJM DAY/PUCO, Troy/county-dominant AND the campus's disclosed intended long-term utility via a proposed 40-yr franchise, first reading only / pending adoption), Piqua
municipal #15095 a full-form filer serving the City + construction power; and the I-75-corridor
...
- [x] A per-jurisdiction County/City GIS connector exists (the known lift — see docs/onboarding.md; verify Miami County **OH**, not another Miami County). (**Parcels done #1483** — `MIAMI_PARCEL_SCHEMA` / county AGOL `parcel_joined`; **zoning done #830** — `PIQUA_ZONING_SCHEMA` / City AGOL `Zoning_Districts_public_view/17`, campus `N44-101770` → `IH`.)
