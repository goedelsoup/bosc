...
- [x] **Per-jurisdiction GIS** — **Miami County parcels WIRED (#1483):** `MIAMI_PARCEL_SCHEMA` over the county AGOL `parcel_joined` FeatureServer (org `wCWf4EGMg4PzHwzA`), `parcels_url` + `gis_parcel` set; same-name trap cleared (situs Piqua OH 45356, FIPS 39109, WKID 3735). Flood = national NFHL (wired). **City of Piqua zoning WIRED (#830):** `PIQUA_ZONING_SCHEMA` over the City AGOL `Zoning_Districts_public_view/17` (org `kZPPWTIJ6kOFJTWc`), `zoning_url` + `gis_zoning` set; situs verified — campus parcel `N44-101770` → `IH` (Industrial Heavy). District catalog committed under `data/reference/troy-piqua-gis/`.
...
**DP&L #4922** (PJM DAY/PUCO, Troy/county-dominant AND the campus's 40-yr-franchise long-term utility), Piqua
