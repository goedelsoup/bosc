...
Living record for the Findlay watershed point (basin: maumee), scaffolded by `watermark onboard findlay`. Check items as you complete them; the site is **not** promoted (`data/sites.yaml` `status`/`selectable`, then `watermark sites sync` — the old `web/src/lib/sites.ts` path is retired and nothing in `web/` is hand-edited) until the gate is clear. **Gate cleared and Findlay promoted 2026-08-13 (#1265).**
...
- [x] **Data-center activity** — **a disclosed, operating facility (#1459).** MARA Holdings, Inc. holds a 15-year take-or-pay lease for the full 150 MW at One Power Co's Findlay Megawatt Hub (MWHub 01) in Allen Township, 30 MW energized — SEC-verified on both sides (One Power Form S-1, EDGAR CIK 2039139; MARA release 2024-11-11). Register: `data/extracted/findlay/data-centers.md`. ⚠️ **MARA Holdings ≠ Marathon Petroleum** (the Findlay-HQ refiner) — an EntityGraph disambiguation guard runs on both. The `[~]` "affirmatively nothing documented / `facility=None`" this line carried is the **2026-06 first-pass finding, superseded**; it is preserved verbatim in the dated self-research section below, which is the record of what was believed then. A separate +300 MW standalone interconnection (S-1, 2024) has **no named customer** and stays `[open]`.
...
- [x] Every written reference value is reviewed against a cited source (no fabricated values). **Reviewed 2026-08-13 for the promotion (#1265).** Three load-bearing claims re-verified against their sources, and one defect found and fixed:
  - **The at-outfall denominator resolves as documented.** `screen_facility` on OH0025135 returns **0.21 cfs (`document`)**, dilution **0.009**, flag `violation` — Ohio EPA's own Table 12 figure, not the retired derived value. The `permits:`-bound entry is reach-keyed (`blanchard river (findlay wpcc outfall, rm 56.42)`) and does not collide with the bare river name, which still resolves to the derived screening proxy 8.67 as intended. Ottawa (OH0026921) remains `no_receiving_water` — its open half, tracked on #415, not this site's.
  - **Every surviving mention of the retired 8.67 cfs is framed as superseded**, never asserted. The sections above are preserved as the record of what was believed, per the corpus's own discipline.
  - **The facility figures are as cited.** The 30–150 MW bracket is the S-1's "Current Capacity 30 MW, Planned Maximum 150 MW" against MARA's 150 MW take-or-pay; the +300 MW standalone interconnection is excluded from the basis and stays `[open]`; the MARA Holdings ≠ Marathon Petroleum guard is intact both directions. No `corrections_to_the_register` block exists in this tree, so the #1992 register-vs-profile divergence has no analogue here.
  - **DEFECT FOUND AND FIXED — the study reported a filed disclosure as a screening estimate.** `_compose_project` derived the IT-load stat's `sub` from the facility domain's instrument test but its `source` from `air_permit_citation` alone, collapsing the four `ItLoadGrounding` grades to a binary. Findlay is the network's first `DISCLOSURE`-grounded facility (grounded by an SEC filing, not an air permit), so it was the first site where the two disagreed: the stat read "screening estimate" directly beneath "bracket grounded by an instrument on the record". Both labels now key on the same predicate and it reads **"filed disclosure"**. Zero blast radius — every other site is either permit-grounded or not `live`. The narrower residue (a `reference`-grade announced ceiling is also labelled "screening estimate" at bowling-green / springfield / van-wert) needs the grade published to the `facility` feed, which is a contract change and is filed separately.
...
- [x] PROMOTION IS A SEPARATE MANUAL EDIT: flip status->live + selectable->true for 'findlay' in `data/sites.yaml` (the identity SSOT — the old `web/src/lib/sites.ts` path is retired; nothing in `web/` is hand-edited), then `watermark sites sync`. onboard never auto-promotes; only one live build (/bosc) exists today. **Done 2026-08-13 (#1265):** Findlay is the **6th** selectable site and the **3rd** at `reference` tier, promoted on **Fort Wayne parity** — an identical readiness block (all five domains `live`) rather than the `case`-tier Troy-Piqua/Sidney line. It is the first promotion whose `facility` is `live` on a filed disclosure instead of a committed air permit.
