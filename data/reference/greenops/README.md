...
### `greenops` — GreenOps — Anthropic Admin usage & cost

Source: Anthropic Admin API — usage_report/messages + cost_report (org usage & cost, by model/workspace) · License: unspecified · Access: keyed · Site scope: basin-shared · Refresh: on-demand

Regenerate: `watermark greenops anthropic`

| file | type | lfs |
| --- | --- | --- |
| `reference/greenops/anthropic-usage.yaml` | application/x-yaml | no |

### `greenops-aws` — GreenOps — AWS costs & estimated carbon

Source: AWS Cost Explorer (ce:GetCostAndUsage) + AWS Sustainability API (GetEstimatedCarbonEmissions, the Customer Carbon Footprint Tool's successor) · License: unspecified · Access: keyed · Site scope: basin-shared · Refresh: on-demand

Regenerate: `watermark greenops aws`
...
Source: derived from the GreenOps connector exports (AWS / GitHub / Anthropic) + the eGRID/WUE factor tables — the modeled usage → electricity → CO2e → water assembly · License: unspecified · Access: public · Site scope: basin-shared · Refresh: on-demand

Regenerate: `watermark greenops footprint --write`

| file | type | lfs |
| --- | --- | --- |
| `reference/greenops/footprint.yaml` | application/x-yaml | no |

### `greenops-github` — GreenOps — GitHub Actions minutes & storage

Source: GitHub enhanced billing platform — /organizations/{org}/settings/billing/usage (Actions minutes by runner + Git LFS/Packages/shared storage, net cost) · License: unspecified · Access: keyed · Site scope: basin-shared · Refresh: on-demand

Regenerate: `watermark greenops github`

| file | type | lfs |
| --- | --- | --- |
| `reference/greenops/github-usage.yaml` | application/x-yaml | no |
